...
#define SAT_PKT_FWD_ADD
//#define SAT_PKT_FWD_NOT_REMOVE
...
void init_sat_network(void) {
    ...
    sat_srv.addr_up.sin_addr.s_addr = INADDR_ANY;//inet_addr(sat_srv.host);
    ...
    sat_srv.addr_down.sin_addr.s_addr = inet_addr(sat_srv.host);
    ...
    rc = bind(sat_srv.sock_up, (struct sockaddr *)&sat_srv.addr_up, sizeof(sat_srv.addr_up));
    if (rc != 0) { 
...
int main(void)
{
...
#ifdef SAT_PKT_FWD_ADD
    parse_hisky_configuration(hisky_cfg_path);
    init_sat_network();
#endif //SAT_PKT_FWD_ADD
    ...
    MSG("INFO: [up] TTN addr:%s port:%s family %d type %d proto %d\n",  host_name, 
                                                                    port_name,
                                                                    q->ai_family,
                                                                    q->ai_socktype,
                                                                    q->ai_protocol);
    ...
    MSG("INFO: [down] TTN addr:%s port:%s family %d type %d proto %d\n",  host_name, 
                                                                    port_name,
                                                                    q->ai_family,
                                                                    q->ai_socktype,
                                                                    q->ai_protocol);
...
void thread_up(void) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        int bytes = 0,nb_pkt = 0;
        MSG("INFO: [up] waiting for packets\n");
        bytes = recvfrom(sat_srv.sock_up, rxpkt, sizeof(struct lgw_pkt_rx_s), 0,
                (struct sockaddr *) &sat_srv.addr_up, &peer_addr_len); 
        nb_pkt += bytes / sizeof(struct lgw_pkt_rx_s);
        // for(int i = 0;i < NB_PKT_MAX;i++)
        // {
        //     bytes = recvfrom(sat_srv.sock_up, &rxpkt[i], sizeof(struct lgw_pkt_rx_s), 0,
        //                     (struct sockaddr *) &sat_srv.addr_up, &peer_addr_len); 
        //     nb_pkt += bytes / sizeof(struct lgw_pkt_rx_s);
        //     MSG("INFO: [up] got %d bytes %d nb_pkt\n",bytes,nb_pkt);
        //     if(bytes == 0)
        //     {
        //         break;
        //     }
...
#ifdef SAT_PKT_FWD_ADD
                MSG("Sending downlink packet");
                sendto(sat_srv.sock_down, (void *)&txpkt, sizeof (struct lgw_pkt_tx_s), 0,
                  (const struct sockaddr *) &sat_srv.addr_down, sizeof(sat_srv.addr_down));
#endif
                pthread_mutex_lock(&mx_meas_dw);
