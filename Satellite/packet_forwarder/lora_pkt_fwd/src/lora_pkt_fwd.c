...
#define LORA_PKT_FWD_ADD
//#define LORA_PKT_FWD_NOT_REMOVE

// #define INIT_MSG_SIZE		32	
// #define PAYLOAD_SIZE		256
// #define RECV_PKT_SIZE(p)	(sizeof(struct lgw_pkt_rx_s) - PAYLOAD_SIZE + sizeof(void*) + p->size)
// #define SEND_PKT_SIZE(p)    (sizeof(struct lgw_pkt_rx_s) - PAYLOAD_SIZE + sizeof(void*) + p->size)

char *hisky_cfg_path = "hisky_conf.json"; 
...
typedef struct {
    ...
    int  sock_up;
    int  sock_down;
    struct sockaddr_in  addr_up;
    struct sockaddr_in  addr_down;
} sat_connection_t;

sat_connection_t sat_srv;	

static void parse_hisky_configuration(const char * conf_file) {
    const char conf_obj_name[] = "hisky_conf";
    JSON_Value *root_val;
    ...
    str = json_object_get_string(conf_obj, "sat_srv_host");
    if (str != NULL) {
        strncpy(sat_srv.host, str, 63);
        MSG("INFO: SAT server hostname or IP address is configured to \"%s\"\n", sat_srv.host);
    } else {
        exit(EXIT_FAILURE);;
    }

    /* get up and down ports */
    val = json_object_get_value(conf_obj, "sat_up_port");
    if (val != NULL) {
        snprintf(sat_srv.port_up, sizeof sat_srv.port_up, "%u", (uint16_t)json_value_get_number(val));
        MSG("INFO: upstream SAT port is configured to \"%s\"\n", sat_srv.port_up);
    } else {
        exit(EXIT_FAILURE);;
    }

    val = json_object_get_value(conf_obj, "sat_down_port");
    if (val != NULL) {
        snprintf(sat_srv.port_down, sizeof sat_srv.port_down, "%u", (uint16_t)json_value_get_number(val));
        MSG("INFO: SAT downstream port is configured to \"%s\"\n", sat_srv.port_down);
    } else {
        exit(EXIT_FAILURE);;
    }
}

void init_sat_network(void)
{
    int enable = 1;
    int rc;

	MSG("Init Sat SrvUP on host %s port: %d\n", sat_srv.host, atoi(sat_srv.port_up));

    sat_srv.addr_up.sin_family = AF_INET; 
    sat_srv.addr_up.sin_addr.s_addr = inet_addr(sat_srv.host);
    sat_srv.addr_up.sin_port = htons(atoi(sat_srv.port_up));

	MSG("Init Sat SrvDown on host %s port: %d\n",sat_srv.host, atoi(sat_srv.port_down));

    sat_srv.addr_down.sin_family = AF_INET; 
    sat_srv.addr_down.sin_addr.s_addr = INADDR_ANY;//inet_addr(sat_srv.host);
    sat_srv.addr_down.sin_port = htons(atoi(sat_srv.port_down));

	if ((sat_srv.sock_up = socket(AF_INET, SOCK_DGRAM, 0)) <= 0) {
       	MSG("ERROR: Lora SrvUP socket creation failed...\n"); 
		exit(EXIT_FAILURE);;
	} 
	if (setsockopt(sat_srv.sock_up, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
		MSG("setsockopt failed... err:%s\n", strerror(errno)); 
	   	exit(EXIT_FAILURE);;
	}

	if ((sat_srv.sock_down = socket(AF_INET, SOCK_DGRAM, 0)) <= 0) {
       	MSG("ERROR: Lora SrvUP socket creation failed...\n"); 
		exit(EXIT_FAILURE);;
	} 

	if (setsockopt(sat_srv.sock_down, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
		MSG("setsockopt failed... err:%s\n", strerror(errno)); 
	   	exit(EXIT_FAILURE);;
	}

    rc = bind(sat_srv.sock_down, (struct sockaddr *)&sat_srv.addr_down, sizeof(sat_srv.addr_down));
    if (rc != 0) { 
		MSG("bind failed...rc %d err:%s\n", rc, strerror(errno)); 
   		exit(EXIT_FAILURE); 
    }
}
...
#ifdef LORA_PKT_FWD_ADD
    parse_hisky_configuration(hisky_cfg_path);
    init_sat_network();
#endif //LORA_PKT_FWD_ADD

#ifdef LORA_PKT_FWD_NOT_REMOVE
...
void thread_up(void) {
...
#ifdef LORA_PKT_FWD_ADD
    struct lgw_pkt_rx_s satrxpkt[NB_PKT_MAX]; /* array containing inbound packets + metadata */
    int pkt_ix = 0;
#endif
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
...
#ifdef LORA_PKT_FWD_ADD
            memcpy(&satrxpkt[pkt_ix],p,sizeof(struct lgw_pkt_rx_s));
            pkt_ix++;
#endif
        }   
        ...
        if(pkt_ix != 0)
        {
            MSG("Sending row %d packet to host",pkt_ix);
            sendto(sat_srv.sock_up, (const char *)satrxpkt, sizeof(struct lgw_pkt_rx_s) * pkt_ix, 0, (const struct sockaddr *) &sat_srv.addr_up,  sizeof(struct sockaddr_in));
            pkt_ix = 0;
            continue;
        }
#endif
...
void thread_down(void) {
    ...
    *(uint32_t *)(buff_req + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        MSG("INFO: Wait for downlink packets\n");
        if (recvfrom(sat_srv.sock_down, (void *)&txpkt, sizeof (struct lgw_pkt_tx_s), 0,
                    (struct sockaddr *) &sat_srv.addr_down, &peer_addr_len) <= 0) {
            MSG("ERROR: Can't get Packet from Sat. Server\n");
            break;
        }
